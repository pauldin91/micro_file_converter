defmodule Core.Validators.Transform do
  ...
  @spec validate(atom() | String.t(), map()) ::
          {:ok, map()} | {:error, map()}
  def validate(transform, props) when is_binary(transform) do
    validate(String.to_existing_atom(transform), props)
  rescue
    ArgumentError -> {:error, %{transform: "Unknown transform"}}
  end

  def validate(transform, props) when is_atom(transform) do
